...
Friend Class OptionDataCRIO
    ...
    Public Sub New()
        ...
        ' Чтобы таймер не был удален при сборке мусора, раскомментировать метод GC.KeepAlive в конце процедурыMain.
    ...
    '    ??? заменить имя переменной в левой колонке “человеческим” именем свойства?
    ' Для этого предназначен атрибут DisplayName:
    '    ??? отобразить расширенную подсказку по свойству в нижнем окне?
    ' Для этого предназначен атрибут Description:
    '    ??? сгруппировать свойства по категориям?
    ' Использовать атрибут Category:
    '    ??? отобразить свойство, недоступное для редактирования?
...
#End Region
    ...
    '    ??? организовать выбор значения из выпадающего списка, формируемого программно?
...
#Region "4. Дополнительно"
    ...
    '    ??? организовать редактирование свойства в собственной форме?
